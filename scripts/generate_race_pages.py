...
<nav class="gl-nav">
  <div class="gl-nav-inner">
    ...
    <ul class="gl-nav-links">
      ...
      <li class="gl-nav-item"><a href="/consulting/"{_active("consulting")}>Consulting</a></li>
...
def renumber_section_kickers(sections: list[str], start: int = 1) -> str:
...
def build_similar_races(race: dict, candidates: list[dict]) -> str:
    ...
    for entry in entries:
        ...
        cards.append(f"""
    <a class="gl-similar-card" data-similar-race="{esc(entry.get('slug', ''))}" data-related-race="{esc(entry.get('slug', ''))}" href="/race/{esc(entry.get('slug', ''))}/">
...
def generate_page(race: dict, all_races: Optional[list[dict]] = None) -> str:
    ...
    sections = renumber_section_kickers([
        vitals, course, interactive, climate, history, series, similar, youtube, sources,
    ], start=3)
...
<body data-race-slug="{esc(slug)}" data-page-format="spine-v2-approved">
<a href="#ratings" class="gl-skip-link">Skip to content</a>
{nav_header}
{breadcrumb}
<div class="gl-page">
{hero}
