...
## Trail-map direction work (Aug 2026)

59. **A token that is both a fill and small text will fail contrast.** `--tape-deep`
was used for the tack, the dotted Grand Classic line, section kickers and inline
links. It measured **4.04:1** on `--gl-paper` and 4.46:1 on the lighter panel —
under the 4.5 floor for small text, though fine as a mark. One value cannot serve
both jobs. Split by role, and check each against its own ground:
`--accent` (decorative fill), `--accent-ink` (text and meaningful lines, ≥4.5:1),
`--accent-on` (text sitting on the accent fill). The same audit caught `--ink-3`
at 2.85:1 wherever it carried muted labels. Guarded by
`tests/test_trailmap.py::test_text_tokens_meet_wcag_aa_on_their_own_ground`,
which parses the shipped token blocks and asserts per direction.

60. **Never hand-type a race figure into markup.** The first cut of the design
preview had `Record 048 of 229`, coordinates, an ascent, `11 sources cited` and
`surveyed 14 March 2026, on skis` typed straight into HTML. Some matched
`race-data`; the survey line, the verification date and the source count matched
*nothing* — those fields do not exist in any of the 229 profiles. On a design
whose whole argument is provenance, an invented survey line is not placeholder
copy, it is the exact failure the design claims to prevent. Fix: the preview is
generated by `scripts/trailmap_preview.py` from `race-data/*.json`, and anything
the schema cannot back is declared in one `PROPOSED_SCHEMA` block naming the
columns a migration must add. A manifest records every rendered value with its
provenance, and a test fails if the proposed set grows.

61. **Counting elements by substring will find the wrapper too.**
`svg.count('class="nw-node')` also matches `class="nw-nodes"`, the group element —
so the database page displayed "92 on marked trails" for several revisions when
the real figure is 91. Derive user-facing counts from the data, not by counting
markup; if you must count markup, anchor the pattern (`<a class="nw-node "`).

62. **A full-viewport `position:fixed` overlay cannot be masked per element.**
The Amundsen ledger rules were a fixed layer with `mix-blend-mode:multiply`, which
ruled straight through the record table it was supposed to sit behind. Textures
that some elements must cover belong on the page container as a background, with
the covering element painted opaque — then a sheet reads as paper laid on the
ledger rather than printed through it.

63. **A direction/theme override will outrank the responsive collapse.**
`[data-dir="amundsen"] .herorow{grid-template-columns:minmax(0,1fr) 420px}` is
specificity (0,2,0) and beat the unscoped `@media (max-width:860px)` rule at
(0,1,0), forcing a 420px fixed column inside a 390px viewport — 82px of
horizontal overflow, on mobile only, in one theme only. Any theme-scoped layout
override must be wrapped in the same breakpoint it is meant to apply at.

64. **Match a generated grid's pitch to whatever rules sit on top of it.** The
ledger was drawn at 27px and the field-record rows were 31px, so the two beat
against each other. Both now use the same 31px pitch.
